...
questions_locales = {
 ...
 questions_locales.each do |q, details|
   ...
   question.question_id = (Question.find_by_sql "Select id from questions where section_id = #{Section.find_by_title(details[:section]).id} and number = #{details[:number]}").first.attributes["id"]
   question.text = details[:text]
   question.guidance = details[:guidance]
   question.save!
 end
