   ...
   if user_signed_in? then
        ...
        respond_to do |format|
          if @comment.save
            ...
            format.html { redirect_to edit_project_plan_path(@project, @plan), status: :found, notice: I18n.t('helpers.notices.comment_created') }
  ...
  def update
    ...
    respond_to do |format|
      if @comment.update_attributes(params[:comment])
        session[:question_id_comments] = @comment.question_id
        format.html { redirect_to edit_project_plan_path(@project, @plan), status: :found, notice: I18n.t('helpers.notices.comment_updated')}
        format.json { head :no_content }
      end
    end
  end
