class Dmptemplate < ActiveRecord::Base
  ...
  def self.funders_templates
    new_org_obejcts = OrganisationType.find_by_name(I18n.t("helpers.org_type.funder", :locale => :en)).organisations
    org_templates = Array.new
  ...
  def self.parent_templates(org_id)
	...
	def self.funders_and_own_templates(org_id)
        ...
        if current_org.organisation_type.name != I18n.t("helpers.org_type.funder", :locale => :en) then
